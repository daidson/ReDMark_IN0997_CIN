...
dct_layer = layers.Conv2D(num_of_filters, (1, 1), activation='linear', padding='same', use_bias=False,
                          trainable=trainable_transform, name='dct')
idct_layer = layers.Conv2D(num_of_filters, (1, 1), activation='linear', padding='same', use_bias=False,
                           trainable=trainable_transform, name='idct')
...
# Encoder
encoder_model = layers.Conv2D(num_of_filters, (1, 1), dilation_rate=1, activation='elu', padding='same',
                              name='enc_conv1')(encoder_input)
encoder_model = conv2d_layer(name='enc_conv2', **conv2d_layer_params)(encoder_model)
if conv_type != 'graph':
    encoder_model = conv2d_layer(name='enc_conv3', **conv2d_layer_params)(encoder_model)
    encoder_model = conv2d_layer(name='enc_conv4', **conv2d_layer_params)(encoder_model)
    encoder_model = conv2d_layer(name='enc_conv5', **conv2d_layer_params)(encoder_model)
encoder_model = idct_layer(encoder_model)
encoder_model = layers.Add(name='residual_add')([encoder_model, l1])

x = layers.Lambda(tf.nn.depth_to_space, arguments={'block_size': block_size}, name='enc_output_depth2space')(
    encoder_model)
...
salt_pepper_attacked = layers.Lambda(salt_pepper_noise, arguments={'salt_ratio': 0.04}, name='salt_pepper_attack')(
    encoder_model)
...
#####################    smoothing_attak  #######################
smoothing_layer = layers.Conv2D(1, (3, 3), dilation_rate=1, padding='same', name='smoothing_attak', use_bias=False,
                                trainable=False)
mean_attacked = smoothing_layer(x)
mean_attacked = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': block_size},
                              name='mean_attack_space2depth')(mean_attacked)
...
sharpenning_attacked = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': block_size},
...
attacked_Iw = layers.Lambda(random_switch, arguments={'prob': attack_prob}, name='Random_selection_of_attacks')(
    attack_list)
...
model.compile(loss={'enc_output_depth2space': ssimmse_loss, 'dec_output_depth2space': 'binary_crossentropy'},
              # 'mean_squared_error'},
...
# %% Training
exp_id = f"{conv_type}"+"_with_deformable_model"
...
output_dir = f"{conv_type}"+"_with_deformable_training_"+f"{training_day}"
